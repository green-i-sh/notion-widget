 ...
 *  per day, for the ?w=calendar Notion embed. Not the dashboard's local
...
export function CalendarWidget() {
  ...
  return (
    <div className="photo-cal-grid" role="grid">
      ...
      {cells.map((iso, i) => {
        if (!iso) return <div key={`pad-${i}`} className="photo-cal-cell empty-cell" />;
        const entry = byDate.get(iso);
        const dayNum = Number(iso.slice(8));
        const cls = `photo-cal-cell${iso === today ? " today" : ""}${entry?.photo ? " has-photo" : ""}`;
        const style = entry?.photo ? { backgroundImage: `url(${entry.photo})` } : undefined;
        ...
        return (
          <a key={iso} href={notionUrl(entry.pageId)} target="_blank" rel="noreferrer" className={cls} style={style}>
            <span className="photo-cal-num">{dayNum}</span>
          </a>
        );
      })}
